...
# `../ComfyUI-GGUF/nodes.py`, registered a synthetic package and called
# `spec.loader.exec_module`. On the machine that built the pack that always
# worked, because ComfyUI-GGUF is installed there. On a stock ComfyUI it raises
# — and it raises during `import`, so ComfyUI reports the WHOLE pack as failing
# to load and not one node of it appears. A pack published to the registry is,
# by definition, mostly installed on machines that are not ours.
#
# The pack already had the right pattern for this and used it twice: SAM2
# (`AnymatixSAM2Loader.load_model`) and controlnet_aux
# (`_ensure_controlnet_aux_importable`) both look for their sibling AT CALL
# TIME and raise a sentence naming what to install. This is the third one,
# brought into line: absent sibling, no GGUF node in the menu, everything else
# works.
#
# `AnymatixUNETLoaderGGUF` cannot subclass a class that may not exist, so the
# subclass is built inside the loader and the module-level name is either the
# real class or None. `__init__.py` registers it only when it is not None.
_gguf_nodes = None
_gguf_import_error: Optional[str] = None


def _load_gguf_nodes():
    """Import the sibling ComfyUI-GGUF package, or explain why it could not be.

    The on-disk folder name contains a hyphen, so it is exposed under the valid
    package name `ComfyUI_GGUF` and registered in sys.modules with a __path__
    before `nodes.py` is executed — otherwise its own relative imports
    (`from .ops import ...`) have no parent package to resolve against.
    """
    global _gguf_nodes, _gguf_import_error
    if _gguf_nodes is not None or _gguf_import_error is not None:
        return _gguf_nodes

    gguf_nodes_path = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "ComfyUI-GGUF", "nodes.py")
    )
    if not os.path.isfile(gguf_nodes_path):
        _gguf_import_error = (
            "ComfyUI-GGUF is not installed next to anymatix-comfy-nodes; "
            "the GGUF UNET loader is unavailable. "
            "Install https://github.com/city96/ComfyUI-GGUF to enable it."
        )
        return None

    custom_nodes_path = os.path.dirname(os.path.dirname(gguf_nodes_path))
    ...
    try:
        ...
        sys.modules[nodes_mod_name] = module
        spec.loader.exec_module(module)
    except Exception as e:
        _gguf_import_error = f"ComfyUI-GGUF is present but could not be loaded: {e}"
        print(f"[ANYMATIX] {_gguf_import_error}")
        return None
...
def _build_gguf_unet_loader():
    """The GGUF UNET loader class, or None when the sibling is absent."""
    module = _load_gguf_nodes()
    if module is None:
        print(f"[ANYMATIX] {_gguf_import_error}")
        return None

    class AnymatixUNETLoaderGGUF(module.UnetLoaderGGUF):
        """Anymatix's twin of ComfyUI-GGUF's UnetLoaderGGUF: the name arrives as
        a path from AnymatixFetcher, and only the basename reaches the sibling."""
...
AnymatixUNETLoaderGGUF = _build_gguf_unet_loader()
